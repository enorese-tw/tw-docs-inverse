...
{
    public class OtrosHaberesFiniquitos
    {
        ...
        {
            string strSql = string.Format("Select MAX(ID) as ID From FNOtrosHaberesFiniquito");
        ...
        public void Grabar(string connectionString)
        {
            ...
            try
            {
                if (ID > -1)
                {
                    strSql = string.Format("INSERT INTO FNOtrosHaberesFiniquito (ID,IDDesvinculacion,Descripcion,Monto) VALUES ({0},{1},'{2}',{3})",
                        ID,
                        IDDesvinculacion,
                        Descripcion,
                        Monto);
            ...
            {
                Utilidades.LogError(connectionString, "FNOtrosHaberesFiniquito.Grabar", ex.Message);
