                ...
                if (sheetExits) {
                    ...
                    if (excelJsonObj.length > 0) {
                        ...
                        `
                            ...
                            <h1 class="new-family-teamwork" style="font-weight: 100; text-align: center; width: 100%; color: rgb(70, 70, 70);">${(excelJsonObj.length > 1) ? "Trabajadores" : "Trabajador"}</h1>
                        `;

                        for (let h = 2; h <= excelJsonObj.length + 1; h++) {
                            templateColumn = "";
                            ...
                            for (let j = 0; j < atob(fileUpload.dataset.columnas).split(':').length; j++) {
                                if (workbook.Sheets[sheetIncorporada][atob(fileUpload.dataset.columnas).split(':')[j] + "1"] !== undefined) {
                                    if (workbook.Sheets[sheetIncorporada][atob(fileUpload.dataset.columnas).split(':')[j] + '' + String(h)] !== undefined) {
                                        xml = xml + workbook.Sheets[sheetIncorporada][atob(fileUpload.dataset.columnas).split(':')[j] + "1"].v +
                                            "='" + workbook.Sheets[sheetIncorporada][atob(fileUpload.dataset.columnas).split(':')[j] + '' + String(h)].v + "' ";
                                        if (j === 0) {
                                            templateColumn = templateColumn + "" + workbook.Sheets[sheetIncorporada][atob(fileUpload.dataset.columnas).split(':')[j] + "1"].v + "";
                                        } else {
                                            templateColumn = templateColumn + "-" + workbook.Sheets[sheetIncorporada][atob(fileUpload.dataset.columnas).split(':')[j] + "1"].v + "";
                                        }
